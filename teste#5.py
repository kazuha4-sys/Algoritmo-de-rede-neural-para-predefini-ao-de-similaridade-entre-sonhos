...
class DreamNetwork(nn.Module):  # Define uma nova classe chamada DreamNetwork que herda de nn.Module
    def __init__(self, input_dim):  # Método de inicialização que define a arquitetura da rede
        ...
        # Camada totalmente conectada com 128 neurônios e input_dim entradas
        self.fc1 = nn.Linear(input_dim, 128)
        # Camada de dropout com probabilidade de 50% para regularização
        self.dropout1 = nn.Dropout(0.5)
        # Outra camada totalmente conectada com 64 neurônios
        self.fc2 = nn.Linear(128, 64)
        # Outra camada de dropout com probabilidade de 50% para regularização
        self.dropout2 = nn.Dropout(0.5)
        # Camada de saída com 1 neurônio (para uma tarefa binária)
    ...
    def forward(self, x):  # Método de propagação direta que define o fluxo de dados na rede
        ...
        # Aplica dropout após a primeira camada
        x = self.dropout1(x)
        # Aplica a função de ativação ReLU na segunda camada
        x = torch.relu(self.fc2(x))
        # Aplica dropout após a segunda camada
        x = self.dropout2(x)
        # Aplica a função de ativação sigmoid na camada de saída para obter uma probabilidade entre 0 e 1
        x = torch.sigmoid(self.fc3(x))
        return x

# Pré-processamento de texto
def preprocess_text(dream):  # Função para pré-processar o texto dos sonhos
    # Obtém a lista de palavras de parada em português
    stop_words = set(stopwords.words('portuguese'))
    # Tokeniza o texto e converte para minúsculas
    tokens = word_tokenize(dream.lower())
    # Filtra palavras que são alfanuméricas e não estão na lista de palavras de parada
...
def train_model(X, y):  # Função para treinar a rede neural com dados e rótulos
    input_dim = X.shape[1]  # Número de características (dimensão de entrada)
    model = DreamNetwork(input_dim)  # Cria uma instância do modelo de rede neural
...
model = train_model(X, np.array(relations))  # Treina o modelo com os dados e relações
